...
export const RepoSelector: React.FC<Props> = ({
  ...
  const getValidationIcon = () => {
    if (validating)
      return (
        <Icon.Loading03Icon className="text-muted-foreground h-4 w-4 animate-spin" />
      );
    if (!validation) return null;
    if (validation.isPrivate)
      return <Icon.Share02Icon className="text-destructive h-4 w-4" />;
    if (!validation.exists)
      return (
        <Icon.CancelCircleHalfDotIcon className="text-destructive h-4 w-4" />
      );
    return <Icon.CheckmarkSquare03Icon className="text-primary h-4 w-4" />;
  };
          ...
          <InputGroupAddon>
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  {loading ? (
                    <Icon.Loading03Icon className="h-4 w-4 animate-spin" />
                  ) : (
        ...
        {validation && (
          ...
          >
            {validation.exists && !validation.isPrivate ? (
              <>
                <Icon.CheckmarkSquare03Icon className="h-3.5 w-3.5" />
